...
$bucketName = $photo_setup['awsBucketName'];

// $uploadFile = dirname(__FILE__).'/DSC_0005.jpg'; // File to upload, we'll use the S3 class since it exists
...
if($setup['amazon_s3_ssl'] == true) { 
	$s3 = new S3(awsAccessKey, awsSecretKey,true);
} else { 
...
//if ($s3->putBucket($bucketName, S3::ACL_PUBLIC_READ)) {
	//echo "<h2>Created bucket {$bucketName}</h2>".PHP_EOL;

	// Put our file (also with public read access)
	if ($s3->putObjectFile($uploadFile, $bucketName, "".$folderName."/".baseName($uploadFile), S3::ACL_PUBLIC_READ)) {
		// echo "S3::putObjectFile(): File copied to {$bucketName}/".$folderName."/".baseName($uploadFile).PHP_EOL;
