...
<?php if($_REQUEST['action'] == "upload") { 
	...
	if(!empty($_REQUEST['folder'])) { 
		$upload_to = $setup['graphics_folder']."/backgrounds/".$_REQUEST['folder']."";
	} else { 
		$upload_to = $setup['graphics_folder']."/backgrounds";
	}
	...
	session_write_close();
	header("location: w-backgrounds-upload-form.php?folder=".$_REQUEST['folder']."&file=".$FILENAME."&refresh=1&noclose=1");
	exit();
}
?>
...
function openBackgroudFolder(folder) { 
	$.get("w-backgrounds.php?noclose=1&folder="+folder, function(data) {
		window.parent.$("#selectbackground").html(data);
	});
}
...
<div><input type="file" name="image" size="40" id="image" ></div>
