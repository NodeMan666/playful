...
$csss = whileSQL("ms_css", "*", "WHERE css_id!='82' AND css_id!='90' ORDER BY css_name ASC ");
while($css = mysqli_fetch_array($csss)) { 
	$fn = 0;
	$qry .= trim($css['css_name'])."||";
	...
	foreach($css2 AS $id => $val) {
		if(!is_numeric($id)) {
			if(($id!=="css2_id")&&($id !== "parent_css_id")&&($id !== "header_code")==true) {
				if($fn>0) {
					$qry .= ", ";
				}
				$qry .= "$id='".addslashes($val)."'";
...
$filename = "sytist-Themes.txt";
$qry = htmlspecialchars($qry);
// $qry = str_replace('&quote;',"ZXXXXXXXXXXXXXX",$qry);
?>
<?php print $qry; ?>
<?php 
exit();
