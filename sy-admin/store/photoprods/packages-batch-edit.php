...
date_id = '<?php print $pp['package_id'];?>';
...
<div class="pc"><h1>Batch Edit Collection Prices</h1></div>
...
if($_REQUEST['action'] == "save") { 
	...
	foreach($items AS $item) {
		if($item > 0) { 
			...
			updateSQL("ms_packages", "package_price='".$_REQUEST['package_price'][$item]."' WHERE package_id='".$item."' "); 
		}
	}
...
<div >
	...
	foreach($prodids AS $id) { 
		if($id > 0) { 
			$pp = doSQL("ms_packages", "*", "WHERE package_id='".$id."' ");
			$total++;
		?>
	...
	<div class="underline" >
		<div class="left"><h3><?php print $pp['package_name']; ?></h3></div>

		<div class="right textright" >
			<div><input type="text" name="package_price[<?php print $pp['package_id'];?>]" value="<?php print $pp['package_price'];?>" id="package_id" class="inputtitle" size="12" ></div>
		</div>
