...
$(document).ready(function(){
	$("#croppreview").imagesLoaded(function() {
	...
	setTimeout(function(){
		...
		jQuery(function($) {
			...
			$('#croppingphoto').Jcrop({
				...
				boxWidth: $("#croppreview").width(),   //Maximum width you want for your bigger images
				boxHeight: $("#croppreview").height()  //Maximum Height for your bigger images

			});    
		});
		$("#croppreview").css({"width":"auto","margin":"auto"});
		$(".jcrop-holder").css({"margin":"auto"});
	},200);
	});
});
$(document).ready(function(){
		// $("#log").show().append(" | "+$("#croppreview").css('width'));
});

function showsize() { 
		$("#log").show().append(" | "+$("#cropemptycontainer").css('width'));
}
	function rotatecrop() {
        jcrop_api = this;
...
<div class="center"  id="photocroparea" style="margin: auto;">
<div id="croppreviewcontainer" style="position: relative; width: 100%; margin: auto; display: block" class="center">
<div id="croppreview" style="margin: auto; display: block; height: auto; text-align:center;" dw="<?php print $size[0];?>" dh="<?php print $size[1];?>">
<img id="croppingphoto" src="<?php if($_REQUEST['color_id'] > 0) { ?><?php tempFolder(); ?><?php print "/sy-photo.php?thephoto=".$pic['pic_key']."|".MD5("pic_pic")."|".MD5($date['date_cat'])."|".$_REQUEST['color_id']."";?><?php } else { print getimagefile($pic,'pic_pic'); } ?>"></div>
...
?>
<div id="cropbuyoptions" class="pc" style="margin-top: 12px;">
<?php if($_REQUEST['disable'] !== "1") { 
	if($prod['pp_width'] !== $prod['pp_height']) { 
?>
<a href="" onclick="cropphotobuy('<?php print $pic['pic_key'];?>','<?php print $prod['pp_id'];?>','<?php print $_REQUEST['con_id'];?>','<?php print $rotate;?>','1'); return false;"><span class="the-icons icon-arrows-cw"></span><?php print _crop_rotate_;?></a>  

<?php 
	} ?>
<a href="" class="" onclick="cropphotobuy('<?php print $pic['pic_key'];?>','<?php print $prod['pp_id'];?>','<?php print $con['pc_id'];?>','0','0','0'); return false;"><span class="the-icons icon-resize-full-alt"></span><?php print _reset_crop_; ?></a>
<?php  } ?>
</div>

<input type="hidden" name="cart_id" id="cart_id" value="<?php print $_REQUEST['cart_id'];?>">
<div>
<input type="hidden" name="x1" id="x1" value="" size="3" class="prod-<?php print $con_id;?>"> 
<input type="hidden" name="y1" id="y1" value="" size="3" class="prod-<?php print $con_id;?>"> 
<input type="hidden" name="x2" id="x2" value="" size="3" class="prod-<?php print $con_id;?>"> 
<input type="hidden" name="y2" id="y2" value="" size="3" class="prod-<?php print $con_id;?>"> 
<input type="hidden" name="w" id="w" value="" size="3" class="prod-<?php print $con_id;?>"> 
<input type="hidden" name="h" id="h" value="" size="3" class="prod-<?php print $con_id;?>"> 
<input type="hidden" name="x1p" id="x1p" value="" size="3" class="prod-<?php print $con_id;?>"> 
<input type="hidden" name="y1p" id="y1p" value="" size="3" class="prod-<?php print $con_id;?>"> 
<input type="hidden" name="x2p" id="x2p" value="" size="3" class="prod-<?php print $con_id;?>"> 
<input type="hidden" name="y2p" id="y2p" value="" size="3" class="prod-<?php print $con_id;?>"> 
<input type="hidden" name="rotate" id="rotate" value="<?php print $_REQUEST['rotate'];?>" size="3" class="prod-<?php print $con_id;?>"> 
