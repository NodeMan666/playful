<div id="authorizenetaim"  class="payoption" <?php if($default !== "authorizenetaim") { ?>style="display: none;"<?php } ?>>
<?php if(!empty($payopt['pay_description'])) { ?>
	...
	<div>
		...
		<div class="pc"><input type="text" name="creditcardanet" size="20" class="cardenter anet"  id="creditcardanet" value=""  errormessage="You did not enter a credit card number"></div>
	</div>
	...
	<div>
		...
		<div class="pc">
			 <select name="monthanet" class="textfield cardenter">
			 ...
			 <select name="yearanet"  class="textfield">
			 <?php 
			 $startYear = date('Y');
			 $endYear = date('Y') + 10;
	...
	<div>
		...
		<div class="pc"><input type="text" name="cvvanet" id="cvvanet" size="4" value=""  class="cardenter anet"  errormessage="You did not enter a CVV number"></div>
	</div>
<div>&nbsp;</div>


<div class="pc" id="cardSubmit-anet">
	<button type="submit" name="continueCheckout" onClick="return checkForm('anet','<?php print $store['require_terms_conditions'];?>');"  class="checkout"><?php print $payopt['pay_text'];?></button>
